...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['AZEN_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Azen) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new Azen({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['AZEN_LOG'] = 'debug';
      const client = new Azen({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['AZEN_LOG'] = 'not a log level';
      const client = new Azen({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'AZEN_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Azen({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['AZEN_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Azen({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['AZEN_BASE_URL'] = 'https://example.com/from_env';
      const client = new Azen({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['AZEN_BASE_URL'] = ''; // empty
      const client = new Azen({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.azen.sh/api/v1');
    });

    test('blank env variable', () => {
      process.env['AZEN_BASE_URL'] = '  '; // blank
      const client = new Azen({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.azen.sh/api/v1');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Azen({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['AZEN_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Azen({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Azen({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Azen({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['AZEN_API_KEY'] = 'My API Key';
    const client = new Azen();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['AZEN_API_KEY'] = 'another My API Key';
    const client = new Azen({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Azen({ apiKey: 'My API Key', fetch: testFetch });
